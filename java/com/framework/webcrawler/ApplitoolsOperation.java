...
public class ApplitoolsOperation extends Report {
	...
	String resourcepath = "." + File.separator + "src" + File.separator + "test" + File.separator + "resources"
			+ File.separator + "webcrawler";
	...
	public void setUpAppliTools() {
		if (applitools) {
			...
			testConfig = (Configuration) new Configuration()
					...
					.addBrowser(
							new DesktopBrowserInfo(viewPortWidth, viewPortHeight, BrowserType.CHROME_ONE_VERSION_BACK))
					.addBrowser(
							new DesktopBrowserInfo(viewPortWidth, viewPortHeight, BrowserType.CHROME_TWO_VERSIONS_BACK))
					...
					.addDeviceEmulation(DeviceName.Nexus_7, ScreenOrientation.PORTRAIT).setForceFullPageScreenshot(true)
					.setViewportSize(new RectangleSize(viewPortWidth, viewPortHeight))
					.setApiKey("nr5ZXjdAisegEDCUKg07qHlgziES1bamI36sVBNR2e0110").setMatchLevel(MatchLevel.CONTENT)
					.setStitchMode(StitchMode.CSS).setSendDom(false).setAppName("Webcrawler")
	...
	void handleTestResults(TestResultContainer summary) {
		if (applitools) {
			...
			if (result == null) {
				...
				node = rpt.createNode(result.getName() + " Browser: --> " + result.getHostApp() + " HostOS: --> "
						+ result.getHostOS());
				extentest.set(node);
	...
	public void setUpAppliToolsNonGrid() {
		...
		if (applitools) {
			// Configuration testConfig = new Configuration();
			// testConfig.setViewportSize( new RectangleSize(viewPortWidth,
			// viewPortHeight));
